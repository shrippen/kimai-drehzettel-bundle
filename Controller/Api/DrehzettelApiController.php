...
final class DrehzettelApiController extends AbstractController
{
    ...
    #[OA\Response(response: 200, description: 'Whether project(+activity)+user+date fall inside an active engagement, and its toggle default for a client-side "film day" UI switch. No engagement is {active: false}, not a 404.')]
    ...
    private function requireEngagement(Request $request, string $date): Engagement
    {
        ...
        if ($engagement === null) {
            throw ApiError::notFound(ApiError::NO_ENGAGEMENT, 'No active engagement for this project, user, activity and date.');
        }
        $this->access->assertView($engagement);

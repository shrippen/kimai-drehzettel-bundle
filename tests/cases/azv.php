...
// Credit per number of shooting days: threshold at 5, block boundary at 20 (PA FAQ: 24 -> 10 h, 26 -> 13 h).
$credit = [];
foreach ([0, 4, 5, 6, 19, 20, 21, 24, 25, 26, 40, 45] as $n) {
    $credit[$n] = Azv::minutes($n);
}
check('azv credit', [0 => 0, 4 => 0, 5 => 150, 6 => 180, 19 => 570, 20 => 600, 21 => 600, 24 => 600, 25 => 750, 26 => 780, 40 => 1200, 45 => 1350], $credit);

// Balance: whole AZV days of 10 h and the rest.
$b25 = new AzvBalance(true, new DateTimeImmutable('2026-01-05'), new DateTimeImmutable('2026-02-06'), 25);
check('azv balance 25 days', [750, 1, 150], [$b25->minutes(), $b25->days(), $b25->openMinutes()]);
...
// 25 shooting days Mon-Fri from Mon 5 Jan 2026: day 5 = 2.5 h, day 20 = 10 h (one AZV day), day 25 = 12.5 h.
...
check('azv 25 day streak', [4 => 0, 5 => 150, 6 => 180, 20 => 600, 21 => 600, 24 => 600, 25 => 750], $series);
$full = $service->balance($tv, $after(25));
check('azv streak balance', [true, '2026-01-05', '2026-02-06', 25, 1, 150], [$full->eligible, $full->countsFrom->format('Y-m-d'), $full->until->format('Y-m-d'), $full->shootingDays, $full->days(), $full->openMinutes()]);
...
check('api azv json', ['engagementId' => null, 'eligible' => true, 'countsFrom' => '2026-01-05', 'date' => '2026-02-06', 'shootingDays' => 25, 'minutes' => 750, 'days' => 1, 'openMinutes' => 150, 'dayMinutes' => 600, 'blockDays' => 20], $json);
...
check('summary azv minutes', [750, null, null], [
...
check('view: azv line', 'drehzettel.azv.pdf_line(%date%=06.02.2026,%hours%=12:30,%shooting%=25,%days%=drehzettel.azv.days(%count%=1))', $view['azv']);

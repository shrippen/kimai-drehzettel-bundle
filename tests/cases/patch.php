...
function patchError(array $body, StreakMode $mode = StreakMode::CALENDAR_WEEK): ?string
{
    try {
        FilmDayPatch::fromArray($body, $mode);
...
$day = travelDay();
FilmDayPatch::fromArray(['breakMinutes' => 30], StreakMode::CALENDAR_WEEK)->applyTo($day);
...
$day = travelDay();
FilmDayPatch::fromArray(['breakMinutes' => null, 'catering' => false, 'category' => null, 'dayType' => 'workday', 'productionDay' => null, 'note' => '  '], StreakMode::CALENDAR_WEEK)->applyTo($day);
check('patch all fields', [null, Catering::NO, null, DayType::WORKDAY, null, null], dayFields($day));

$day = new FilmDay();
FilmDayPatch::fromArray(['breakMinutes' => '45', 'catering' => true, 'category' => 'sunday', 'dayType' => 'travel', 'productionDay' => 7, 'note' => ' Nachtdreh '], StreakMode::CALENDAR_WEEK)->applyTo($day);
...
check('patch production day consecutive', [null, 'productionDay must be an integer from 1 to 999.'], [patchError(['productionDay' => 999], StreakMode::CONSECUTIVE), patchError(['productionDay' => 1000], StreakMode::CONSECUTIVE)]);
...
$day = travelDay();
FilmDayPatch::fromArray(['extraPayCents' => 5000], StreakMode::CALENDAR_WEEK)->applyTo($day);
check('patch extra pay', [5000, 0, 'Reise'], [$day->getExtraPayCents(), $day->getBreakMinutes(), $day->getNote()]);
FilmDayPatch::fromArray(['note' => 'x'], StreakMode::CALENDAR_WEEK)->applyTo($day);
check('patch keeps extra pay', 5000, $day->getExtraPayCents());
FilmDayPatch::fromArray(['extraPayCents' => null], StreakMode::CALENDAR_WEEK)->applyTo($day);
...
$day = travelDay();
FilmDayPatch::fromArray(['shootingDayNumber' => 37], StreakMode::CALENDAR_WEEK)->applyTo($day);
...
FilmDayPatch::fromArray(['shootingDayNumber' => '38'], StreakMode::CALENDAR_WEEK)->applyTo($day);
check('patch shooting day digit string', 38, $day->getShootingDayNumber());
FilmDayPatch::fromArray(['shootingDayNumber' => null], StreakMode::CALENDAR_WEEK)->applyTo($day);
